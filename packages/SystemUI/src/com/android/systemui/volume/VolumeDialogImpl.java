...
public class VolumeDialogImpl implements VolumeDialog,
    ...
    private boolean shouldBeVisibleH(VolumeRow row, VolumeRow activeRow) {
        ...
        if (!mShowActiveStreamOnly) {
            ...
            if (mExpanded && isExpandableRow(row)) {
                return true;
            }
    ...
    private void updateRowsH(final VolumeRow activeRow, boolean animate) {
        ...
        for (final VolumeRow row : mRows) {
            ...
            if ((shouldBeVisible || isExpandableRow(row))
            ...
            if (row.view.isShown() || isExpandableRow(row)) {
        ...
        if (outmostVisibleRowIndex > -1 && outmostVisibleRowIndex < Short.MAX_VALUE) {
            ...
            // Move the expandable rows
            for (final VolumeRow row : mRows) {
                if (row == mDefaultRow && !row.defaultStream) {
                    float translationX = getTranslationForPanelLocation()
                            * (mExpanded ? 0 : getExpandableRowsExtraSize());
                    if (animate) {
                        row.view.animate()
                                .translationX(translationX)
                                .setDuration(mDialogShowAnimationDurationMs)
                                .setInterpolator(mExpanded ? new DecelerateInterpolator()
                ...
                float distance =
                        lastVisibleChildLocation[0] - (locInWindow[0] - row.view.getTranslationX());

                if (animate) {
                    // Avoid no animation
                    if (mExpanded && row.view.getTranslationX() == 0f) {
                        row.view.setTranslationX(distance);
                    }
                    ...
                    row.view.animate()
                            ...
                            .setListener(new AnimatorListenerAdapter() {
                                ...
                                public void onAnimationEnd(Animator animation) {
                                    ...
                                    // TODO can this be done after all rows finished animating?
                    ...
                    row.view.setVisibility(mExpanded ? View.VISIBLE : View.INVISIBLE);
                }
            }
        }
